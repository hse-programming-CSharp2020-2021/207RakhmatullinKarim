...
class Program
{
    ...
    static void Main()
    {
        ...
        BeforeFormat(fileName, enc, linesCount);
        ...
        NewMethod(fileName, messages);
    }

    private static void NewMethod(string fileName, string[] messages)
    {
    ...
    private static void BeforeFormat(string fileName, Encoding enc, int linesCount)
    {
        Console.WriteLine("Переписка до форматирования:");
        string[] arrOFLines = new string[linesCount];
        for (int i = 0; i < linesCount; i++)
        {
            RandomSentence(ref arrOFLines, i); // Добавляем в сообщение точку и перенос на следующую строку.
            Console.WriteLine(arrOFLines[i]);
        }
        File.WriteAllLines(fileName, arrOFLines, enc);
    }

    private static void RandomSentence(ref string[] arrOFLines, int i)
    {
